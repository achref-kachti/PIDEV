...
import javafxmldesktop.AfficherMatrielPanier;
import javafxmldesktop.Panier;
/**
 *
 * @author sarah
 */
...
//int del=-1;
 ...
 try {
    ...
    public List<AfficherMatrielPanier> trierach(int id) throws SQLException{
      List<AfficherMatrielPanier> arr=new ArrayList<>();
      ste = con.createStatement();
      String sql = ("select *  from panier inner join materiel on panier.refmat=materiel.refmat inner join users on panier.iduser=users.id where users.id ='"+id+"' order by panier.qtach " );
...
public Paragraph pdf(int id)throws SQLException, FileNotFoundException, DocumentException{
    String filename="E:\\JavaFXmlDesktop chourou\\pdf\\facture.pdf";
