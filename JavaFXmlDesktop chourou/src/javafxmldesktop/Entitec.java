...
class Evenementc
{
        ...
        public int supprimer_evenement(int id) throws SQLException
    {
        ...
        return res;
          ...
          public int supprimer_evenement_depasses() throws SQLException
    {
        int res=-1;
        res=state.executeUpdate("delete from evenement where date_debut <=sysdate()");
        return res;
...
class Reservationc
{
    ...
    Statement state;
     ...
     public Evenement recuperer_evenement_res(int id) throws SQLException
     {
               ResultSet result=null;
               Evenement e=new Evenement();
     ...
     public int nb_jour_restant (int idr) throws SQLException
     {
             
        ResultSet result=null;
               Evenement e=new Evenement();
               int i=1; 
        result= state.executeQuery("SELECT datediff(date_debut,sysdate()) FROM reservation_evenement inner join evenement on reservation_evenement.id_evenement=evenement.id where reservation_evenement.id="+idr);
        while (result.next())
        {
                  i=result.getInt(1);
        }
     ...
     {
             int result=-1;
             int nb=this.nb_jour_restant(idr);
             if(nb<=1)
             {
                    Alert alert = new Alert(Alert.AlertType.WARNING);
                    alert.setTitle("delai pour suppression depassé");
                    alert.setHeaderText("vous ne pouvez plus annuler la reservation");
                    alert.setContentText("delai depassé ");
                    alert.showAndWait();
     ...
     public boolean ajouter_reservation(Reservation r ) throws SQLException
     {
            ...
            //Evenement e=this.recuperer_evenement_res(r.getId_evenement());
             ...
             if(total_restant<1)
             {
              ...
              //information pour l'envoi du mail et du sms
              String mail_to =r1.getEmail();
              LocalDate date1=r1.getDate_debut().toLocalDate();
              LocalDate date2=r1.getDate_fin().toLocalDate();
             String d1= date1.getDayOfWeek().getDisplayName(TextStyle.FULL, Locale.FRANCE)+"_"+date1.getDayOfMonth()
                     +"_"+date1.getMonth().getDisplayName(TextStyle.FULL, Locale.FRANCE)
                         +"_"+date1.getYear();
             String d2= date2.getDayOfWeek().getDisplayName(TextStyle.FULL, Locale.FRANCE)+"_"+date2.getDayOfMonth()
                     +"_"+date2.getMonth().getDisplayName(TextStyle.FULL, Locale.FRANCE)
                         +"_"+date2.getYear();
             
             System.out.println(d1+"//"+d2);
              
              String message_mail="<h1>bonjour " +r1.getNom()+" votre reservation au evenement:"+r1.getTitre()+" du "+d1+" au "
                    +d2+" est effectuée avec succes,Prix: "+r1.getPrix()+"DT"+"</h1>";
               String message_sms="bonjour_" +r1.getNom()+"_votre_reservation_au_evenement_:"+r1.getTitre()
                       + "_du/"+d1+"_au_/"
                    +d2+"est_effectuée_avec_succes,Prix:"+r1.getPrix()+"DT";
              int telephone_user=r1.getTelephone();
              
              //envoi du mail
      ///* 
   Email email = new Email("youssefaziz.zghal@gmail.com","youssef2010");
        
        try {
            email.setFrom("youssefaziz.zghal@gmail.com", "Reservation");
        ...
        try {
            ...
            email.setContent(message_mail,"text/html");
        } catch (MessagingException ex) {
            Logger.getLogger(Reservationc.class.getName()).log(Level.SEVERE, null, ex);
        }
 ...
 try
{
 System.out.println(message_sms);
URL adr = new URL("http://localhost/envoyersms1.php?telephone="+telephone_user+"&message="+message_sms);

URLConnection connex = adr.openConnection();
...
{
System.out.println("Probléme d'entrée sortie");
e.printStackTrace();
}          
